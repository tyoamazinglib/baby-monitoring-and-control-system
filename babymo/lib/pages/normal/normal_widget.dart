...
class NormalWidget extends StatefulWidget {
  ...
  @override
  _NormalWidgetState createState() => _NormalWidgetState();
}
...
class _NormalWidgetState extends State<NormalWidget> {
  ...
  void initState() {
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      _model.normal = InstantTimer.periodic(
        duration: Duration(milliseconds: 2500),
        callback: (timer) async {
          _model.crystatusnormal = await queryStatusRecordOnce(
            singleRecord: true,
          ).then((s) => s.firstOrNull);
          _model.cry = await queryCryStatusRecordOnce(
            singleRecord: true,
          ).then((s) => s.firstOrNull);
          if (_model.crystatusnormal!.statusSids == true) {
            _model.normal?.cancel();
            triggerPushNotification(
              notificationTitle: 'Your Baby is Danger',
              notificationText: 'Please Take Care your baby wih carefuly!',
              notificationSound: 'default',
              userRefs: [currentUserReference!],
              initialPageName: 'Danger',
              parameterData: {},
            );

            context.pushNamed('Danger');
          } else if (_model.cry!.statusCry == true) {
            _model.normal?.cancel();
            triggerPushNotification(
              notificationTitle: 'Your Baby is Crying!',
              notificationText: 'Take Care Your Baby! ',
              notificationSound: 'default',
              userRefs: [currentUserReference!],
              initialPageName: 'Awake',
              parameterData: {},
            );

            context.pushNamed('Awake');
          } else {
            return;
          }
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: Scaffold(
        key: scaffoldKey,
        backgroundColor: Color(0xFF4ECB76),
        drawer: Container(
          width: 120.0,
          child: Drawer(
            elevation: 15.0,
